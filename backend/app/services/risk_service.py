...
class RiskService:
    ...
    def calculate_all_risks(
        ...
        """
        Calculate and save ML-based risk for every employee.

        This intentionally reuses calculate_risk() so that the
        exact same 24-feature pipeline, scaler, Isolation Forest
        model, and calibration are used for individual and bulk
        calculations.
        """

        # Load the saved ML artifacts once before processing.
        cls._load_ml_artifacts()
        ...
        for index, user_id in enumerate(employee_ids, start=1):
            ...
            try:
                ...
                if risk:
                    ...
                    risk.behavioral_anomalies = float(
                        result["behavioral_anomalies"]
                    )
                    risk.privilege_misuse = float(
                        result["privilege_misuse"]
                    )
                    risk.data_access_violations = float(
                        result["data_access_violations"]
                    )
                    risk.access_pattern_deviations = float(
                        result["access_pattern_deviations"]
                    )
                    risk.historical_security_events = float(
                        result["historical_security_events"]
                    )
                ...
                # Commit periodically so a long run does not keep
                # one huge transaction open.
                if index % 50 == 0:
                    db.commit()
                    print(
                        f"Risk calculation progress: "
                        f"{index}/{total}"
                    )
