...
PLATFORM_USERS = [
    {
        "email":    "admin@cyberai.internal",
        "password": "CyberAdmin2026!",
        "role":     RoleEnum.ADMINISTRATOR,
        "label":    "Administrator",
    },
    {
        "email":    "manager@cyberai.internal",
        "password": "Manager2026!",
        "role":     RoleEnum.SECURITY_MANAGER,
        "label":    "Security Manager",
    },
    {
        "email":    "soc@cyberai.internal",
        "password": "SocEng2026!",
        "role":     RoleEnum.SOC_ENGINEER,
        "label":    "SOC Engineer",
    },
    {
        "email":    "analyst@cyberai.internal",
...
async def main() -> None:
    ...
    try:
        t = await asyncio.wait_for(seed_mongo(), timeout=5.0)
        log.info("  Done — %d telemetry event(s) inserted.", t)
    except Exception as e:
        log.warning("  MongoDB seed skipped (offline or not reachable): %s", e)
        log.info("  CYBER AI platform will operate using relational store (cyberai.db).")
