...
const viewTitles = {
  ...
  intelligenceView: "Behavioral intelligence",
  investigationsView: "Threat investigations",
};
...
async function loadIntelligence() {
  const [anomalies, risk, ueba] = await Promise.all([
    api("/api/analytics/anomalies"), api("/api/analytics/risk"), api("/api/analytics/ueba"),
  ]);
  $("#anomaliesList").innerHTML = anomalies.length ? anomalies.slice(0, 10).map((item) => `<div class="activity-item"><div><strong>${escapeHtml(item.category)}</strong><span>${escapeHtml(item.explanation)}</span></div><span class="severity-badge ${levelClass(item.severity)}">${escapeHtml(item.severity)}</span></div>`).join("") : `<p class="form-note">Run analysis to generate findings.</p>`;
  $("#peerGroupsList").innerHTML = ueba.peer_groups.map((item) => `<div class="activity-item"><div><strong>${escapeHtml(item.peer_group)}</strong><span>${escapeHtml(item.employees)} employees, ${escapeHtml(item.elevated)} elevated</span></div><span class="risk-badge medium">${escapeHtml(item.average_risk)} avg</span></div>`).join("");
  $("#riskTable").innerHTML = risk.map((item) => `<tr><td>${escapeHtml(item.full_name)}<small>${escapeHtml(item.employee_id)}</small></td><td>${escapeHtml(item.department)}</td><td>${escapeHtml(item.risk_score)}</td><td><span class="risk-badge ${levelClass(item.risk_level)}">${escapeHtml(item.risk_level)}</span></td><td>${escapeHtml(item.status)}</td></tr>`).join("");
}

async function loadInvestigations() {
  const items = await api("/api/analytics/investigations");
  $("#investigationsList").innerHTML = items.length ? items.map((item) => `<div class="activity-item"><div><strong>${escapeHtml(item.case_ref)} - ${escapeHtml(item.title)}</strong><span>${escapeHtml(item.notes || "No notes")}</span></div><span class="severity-badge ${levelClass(item.priority)}">${escapeHtml(item.status)}</span></div>`).join("") : `<p class="form-note">No investigations opened.</p>`;
}
...
function renderEmployeeOptions() {
  ...
  $("#investigationEmployeeSelect").innerHTML = options;
}

async function runAnalytics() {
  try {
    const result = await api("/api/analytics/run", { method: "POST" });
    await Promise.all([loadIntelligence(), loadDashboard(), loadEmployees()]);
    showToast(`${result.anomalies_created} findings generated.`);
  } catch (error) { showToast(error.message); }
}

async function importCert() {
  try {
    const result = await api("/api/analytics/datasets/import-cert", { method: "POST" });
    await Promise.all([loadIntelligence(), loadDashboard(), loadEmployees(), loadActivity()]);
    showToast(`${result.events || 0} CERT events imported.`);
  } catch (error) { showToast(error.message); }
}
...
async function createInvestigation(event) {
  ...
  try {
    await api("/api/analytics/investigations", { method: "POST", body: JSON.stringify(Object.fromEntries(new FormData(form).entries())) });
    form.reset();
    $("#investigationStatus").textContent = "Investigation opened.";
    await loadInvestigations();
  } catch (error) { $("#investigationStatus").textContent = error.message; }
}
...
function bindEvents() {
  ...
  $("#runAnalyticsButton").addEventListener("click", runAnalytics);
  $("#importCertButton").addEventListener("click", importCert);
  $("#investigationForm").addEventListener("submit", createInvestigation);
