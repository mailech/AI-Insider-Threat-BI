...
import { api, deriveLogSources } from "@/lib/api/client";
import { ActivityLog, LogSource } from "@/lib/types";
import { Search, Database, RefreshCw, Settings2, Loader2, AlertCircle } from "lucide-react";
...
export default function ActivityMonitoringPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      // One authoritative activity request. Sources are a summary of that
      // response, so this avoids the former second GET /activity per load.
      const nextLogs = await api.getActivityLogs();
      if (version !== requestVersion.current) return;
      const nextSources = deriveLogSources(nextLogs);
      setLogs(nextLogs); setSources(nextSources);
      setSelectedSource((current) => current || nextSources[0] || null);
    } catch (reason) {
      if (version === requestVersion.current) setError(reason instanceof Error ? reason.message : 'Failed to load activity logs');
    } finally {
  ...
  const handleTestConnection = async () => {
    ...
    try { const result = await api.testLogSourceConnection(selectedSource?.id || ""); setTestResult(result.success ? "success" : "error"); }
    catch { setTestResult("error"); } finally { setIsTesting(false); }
  };
        ...
        <Card className="!p-0 border-0 bg-transparent flex flex-col">
          ...
          <div className="bg-onyx border border-graphite border-b-0 p-4 flex flex-col md:flex-row gap-4 items-end">
            <div className="w-full md:w-48 flex flex-col mb-4">
              ...
              <select value={activityType} onChange={(event) => setActivityType(event.target.value)} className="w-full bg-onyx text-bone text-body rounded-sm px-4 py-[11px] border border-slate focus:border-signal-lime outline-none appearance-none cursor-pointer">
            ...
            <div className="w-full md:w-64">
              <Input label="Search Employee or IP" placeholder="Search..." value={search} onChange={(event) => setSearch(event.target.value)} className="!mb-0" />
            </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                logs.filter((log) => (!activityType || log.activityType === activityType) && (!search || `${log.employeeId} ${log.ip} ${log.source}`.toLowerCase().includes(search.toLowerCase()))).map((log) => (
                  <TableRow key={log.id}>
