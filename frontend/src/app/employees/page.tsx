...
import { UserPlus, Download, Building2, Loader2, WifiOff, AlertCircle } from "lucide-react";
...
export default function EmployeesPage() {
  ...
  React.useEffect(() => {
    ...
    ]).then(([empResult, riskResult]) => {
      if (empResult.status === 'fulfilled') {
        setEmployees(empResult.value);
      } else {
        setError(empResult.reason?.message || 'Failed to load employees');
      }
      ...
      <Card className="!p-0 border-0 bg-transparent flex flex-col">
        ...
        <div className="bg-onyx border border-graphite border-b-0 p-4 flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="w-full md:w-48 flex flex-col mb-4">
            ...
            <select value={departmentFilter} onChange={(event) => setDepartmentFilter(event.target.value)} className="w-full bg-onyx text-bone text-body rounded-sm px-4 py-[11px] border border-slate focus:border-signal-lime outline-none appearance-none cursor-pointer">
              <option value="">All Departments</option>
              {Array.from(new Set(employees.map((employee) => employee.department))).sort().map((department) => <option key={department} value={department}>{department}</option>)}
            </select>
          </div>
          <div className="w-full md:w-48 flex flex-col mb-4">
            <label className="font-sans text-[11px] uppercase tracking-[0.18em] text-ash mb-2">
            ...
            <select value={accessLevelFilter} onChange={(event) => setAccessLevelFilter(event.target.value)} className="w-full bg-onyx text-bone text-body rounded-sm px-4 py-[11px] border border-slate focus:border-signal-lime outline-none appearance-none cursor-pointer">
              <option value="">All Levels</option>
              {Array.from(new Set(employees.map((employee) => employee.accessLevel))).sort().map((accessLevel) => <option key={accessLevel} value={accessLevel}>{accessLevel}</option>)}
            </select>
          </div>
          ...
          <TableBody>
            {loading ? (
              ...
              employees.filter((employee) =>
                (employee.name.toLowerCase().includes(searchTerm.toLowerCase()) || employee.id.toLowerCase().includes(searchTerm.toLowerCase())) &&
