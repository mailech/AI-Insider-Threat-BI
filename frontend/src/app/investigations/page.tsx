...
import { Search, Plus, Clock, FileText, MessageSquare, ChevronRight, X, Loader2, AlertTriangle } from "lucide-react";
...
export default function InvestigationsPage() {
  ...
  const handleStatusChange = async (invId: string, newStatus: InvestigationStatus) => {
    ...
    try {
      if (!await api.updateInvestigationStatus(invId, newStatus)) throw new Error("Unable to update investigation.");
      const updated = await api.getInvestigations();
      setInvestigations(updated);
      if (selectedInv?.id === invId) setSelectedInv(updated.find(i => i.id === invId) || null);
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to update investigation."); }
  ...
  const handleAddNote = async () => {
    ...
    setIsSubmitting(true);
    try {
      if (!await api.addInvestigationNote(selectedInv.id, user.email, noteText.trim())) throw new Error("Unable to add note.");
      const updated = await api.getInvestigations(); setInvestigations(updated);
      setSelectedInv(updated.find(i => i.id === selectedInv.id) || null); setNoteText("");
  ...
  const handleCreateInvestigation = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try { await api.createInvestigation({
      ...
      riskBand: "HIGH",
      severity: formData.get("severity") as "Informational" | "Low" | "Medium" | "High" | "Critical",
      description: formData.get("description") as string,
      relatedAnomalyIds: [],
    });
      const updated = await api.getInvestigations(); setInvestigations(updated); setIsCreateModalOpen(false);
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to create investigation."); }
    finally { setIsSubmitting(false); }
  };
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-10">
                      <div className="flex flex-col items-center gap-3">
                        <AlertTriangle className="w-6 h-6 text-validation-error" />
                        <span className="text-validation-error">{error}</span>
                        <Button variant="outline" className="!py-1.5 !px-3 !text-[11px]" onClick={() => {
                          setLoading(true);
                          setError(null);
                          api.getInvestigations()
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Create New Investigation">
        <form onSubmit={handleCreateInvestigation} className="flex flex-col gap-4 mt-2">
          ...
          <div className="flex flex-col w-full mb-4">
            <label className="font-sans text-[11px] uppercase tracking-[0.18em] text-ash mb-2">Severity</label>
            <select name="severity" defaultValue="High" className="w-full bg-onyx text-bone text-body rounded-sm px-4 py-3 border border-slate focus:border-signal-lime outline-none">
