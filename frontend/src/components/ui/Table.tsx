...
export function TableActionMenu({
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      <button type="button" aria-expanded={open} onClick={() => setOpen(!open)} className="p-1 rounded-sm text-fog hover:text-bone hover:bg-graphite transition-colors outline-none focus:glow-lime">
        <MoreVertical className="w-4 h-4" />
      </button>
      {open && <div className="absolute right-0 top-full mt-1 w-32 bg-onyx border border-graphite z-[60] shadow-lg">
        <div className="py-1">
          {options.map((opt, i) => (
            <button
              key={i}
