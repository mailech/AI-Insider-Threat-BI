 ...
 * Defines granular security permissions across the CYBER AI frontend application
...
/**
 * Get active user role from localStorage with fallback
 */
export function getActiveRole(defaultRole: string = 'ADMINISTRATOR'): string {
  if (typeof window === 'undefined') return defaultRole;
  return localStorage.getItem('cyber_active_role') || defaultRole;
}

/**
 * Set active user role and dispatch custom event for live UI reactivity
 */
export function setActiveRole(role: string): void {
  if (typeof window === 'undefined') return;
  localStorage.setItem('cyber_active_role', role);
  window.dispatchEvent(new CustomEvent('cyber-role-changed', { detail: role }));
}
