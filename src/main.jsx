...
// Employee records below are a small synthetic sample styled after the CERT Insider Threat Dataset (r4.2)
// field categories: logon.csv, device.csv, http.csv, email.csv, file.csv. This is NOT the actual CERT
// dataset (that is a restricted, multi-GB research release available only on request from CMU SEI) -
// it only mirrors its log-source structure so the project is realistically modeled on it. See README.
const namedSeed=[
...
const alerts=[
{id:"INC-2048",title:"Mass file download outside baseline",employee:"Ravi Menon",employeeId:"EMP-1042",severity:"Critical",desc:"184 files downloaded in 11 minutes — 8.7× above baseline.",riskScore:91,confidence:94.2,assessment:"Potential data exfiltration",assessmentDetail:"The sequence is 8.7× above Ravi Menon's historical file-access baseline.",timeline:[["02:14 AM","Unusual login"],["02:18 AM","Restricted folder access"],["02:23 AM","184 files downloaded"],["02:25 AM","External connection"],["Model","Recommended human investigation"]]},
{id:"INC-2049",title:"After-hours authentication anomaly",employee:"Sneha Krishnan",employeeId:"EMP-0871",severity:"High",desc:"New device detected outside the established login pattern.",riskScore:68,confidence:81.5,assessment:"Possible credential compromise",assessmentDetail:"Login originated from a device and location never seen before for this account.",timeline:[["11:42 PM","Login from unrecognized device"],["11:43 PM","New IP address, different city"],["11:45 PM","MFA challenge passed"],["11:46 PM","Session began - Finance systems accessed"],["Model","Recommended password reset and review"]]},
{id:"INC-2050",title:"Restricted folder access",employee:"Arjun Joseph",employeeId:"EMP-0318",severity:"High",desc:"Accessed Finance/Payroll outside normal role scope.",riskScore:84,confidence:88.0,assessment:"Unauthorized access attempt",assessmentDetail:"This employee's role (Account Executive) has no business need for Finance/Payroll systems.",timeline:[["09:12 AM","Standard login"],["09:20 AM","Navigated to Finance/Payroll folder"],["09:21 AM","Opened 3 salary records"],["09:24 AM","Access denied on 4th record - escalated"],["Model","Recommended access review with manager"]]},
{id:"INC-2051",title:"USB activity increased",employee:"Priya Nair",employeeId:"EMP-1129",severity:"Medium",desc:"Removable-media activity is 3.2× above baseline.",riskScore:18,confidence:52.0,assessment:"Likely benign, low confidence",assessmentDetail:"Slight increase in USB usage, but well within normal variance for this role.",timeline:[["02:00 PM","USB device connected"],["02:05 PM","Files copied to device"],["02:10 PM","USB safely ejected"],["Model","No action needed - monitor only"]]}
];
...
function Login({setUser}){
 ...
 const go=e=>{
  ...
  // fixed Admin / Analyst / default Employee accounts
  let u=Object.values(demo).find(x=>x.email===email&&x.password===pass);
  // any other employee in the dataset (seed) or added later by Admin can also sign in with their own login
  if(!u){
 ...
 <small className="secure">RBAC enabled · Demo environment</small></div>
...
// header search - looks across employees and alerts, click a result to jump straight to it
function GlobalSearch(){
 ...
 const empResults=active?all.filter(e=>e.name.toLowerCase().includes(term)||e.id.toLowerCase().includes(term)||e.dept.toLowerCase().includes(term)).slice(0,5):[];
...
function Dashboard({user}){
 const custom=JSON.parse(localStorage.getItem("sq_employees")||"[]");
 const removed=JSON.parse(localStorage.getItem("sq_deleted")||"[]");
 const resolved=JSON.parse(localStorage.getItem("sq_resolved_alerts")||"[]");
 const all=[...custom,...seed].filter(e=>!removed.includes(e.id));
 const openAlerts=alerts.filter(a=>!resolved.includes(a.id));
 ...
 <Panel><div className="ph"><div><label>ATTENTION REQUIRED</label><h2>Priority alerts</h2></div><Link to="/alerts">View all →</Link></div>{openAlerts.slice(0,3).map(a=><div className="alert-row" key={a.id}><i>!</i><div><b>{a.title}</b><small>{a.employeeId} · {a.employee}</small></div><Pill status={a.severity}/></div>)}{openAlerts.length===0&&<p style={{color:"#5e7672",fontSize:"9px",padding:"6px 0"}}>All alerts resolved.</p>}</Panel>
...
// Base URL of the small Flask API in /server that serves insider_threat_model.joblib.
// Run it separately (see README) - if it isn't running, the rest of the app still works fine,
// employees just get saved with a baseline score of 0 like before.
const MODEL_API="http://localhost:5000";
// These example values were checked directly against the trained model - "normal" scores as
// normal, "suspicious" scores as an anomaly - since the model expects large cumulative counts
// (totals over the whole monitoring period), not small daily numbers.
const NORMAL_EXAMPLE={logon_count:855,after_hours_logon_count:74,usb_connect_count:405,file_copy_count:446,email_count:2630};
const SUSPICIOUS_EXAMPLE={logon_count:1026,after_hours_logon_count:1113,usb_connect_count:8108,file_copy_count:11140,email_count:263};
const featureLogSource={logon_count:"logon.csv",after_hours_logon_count:"logon.csv",usb_connect_count:"device.csv",file_copy_count:"file.csv",email_count:"email.csv"};
// rough realistic ranges for each feature, based on the trained model's own scaler stats -
// used only to auto-fill plausible random values, not for scoring itself
const FEATURE_RANGES={logon_count:[300,1500],after_hours_logon_count:[0,600],usb_connect_count:[0,1800],file_copy_count:[0,2200],email_count:[400,4800]};
function randomFeatures(){
 const out={};
 Object.entries(FEATURE_RANGES).forEach(([k,range])=>{out[k]=Math.round(range[0]+Math.random()*(range[1]-range[0]))});
 return out;
}
...
function AddEmployee(){
 ...
 const runPrediction=async()=>{
  ...
  try{
   ...
   const res=await fetch(MODEL_API+"/api/predict",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)});
   if(!res.ok)throw new Error("model server error");
   ...
   toast("Couldn't reach the AI model server - is server/app.py running on localhost:5000?","error");
  }finally{
   setPredicting(false);
  }
 };
 ...
 const save=e=>{
  ...
  const activity=prediction?(prediction.is_anomaly?"Flagged by AI model - unusual behavior pattern":"AI model: baseline behavior, no anomalies detected"):"New employee profile";
  let logSource="logon.csv";
  if(prediction){
   const maxKey=Object.keys(feat).reduce((a,b)=>(Number(feat[b])||0)>(Number(feat[a])||0)?b:a,"logon_count");
   logSource=featureLogSource[maxKey];
  }
  const record={...f,id:"EMP-"+Math.floor(1000+Math.random()*8999),score,activity,logSource,password:pwd};
 ...
 if(created){
  return <div className="content"><Header eyebrow="ADMIN CONTROL" title="Employee created" desc="Share these sign-in details with the new employee - the password is shown only once."/>
   <Panel>
    ...
    {prediction&&<p style={{fontSize:"9px",color:"#8da19e",margin:"14px 0 0"}}>Initial risk score set by the AI model: <b style={{color:"#dce8e5"}}>{created.score}/100</b> ({prediction.risk_level}).</p>}
 ...
 <Panel>
  ...
  <div className="actions" style={{justifyContent:"flex-start"}}>
   <button type="button" className="secondary" onClick={()=>setFeat(NORMAL_EXAMPLE)}>Fill example: normal</button>
   <button type="button" className="secondary" onClick={()=>setFeat(SUSPICIOUS_EXAMPLE)}>Fill example: suspicious</button>
   <button type="button" className="secondary" onClick={()=>setFeat(randomFeatures())}>Randomize activity data</button>
   <button type="button" className="primary" onClick={runPrediction} disabled={predicting}>{predicting?"Scoring...":"Predict risk with AI model"}</button>
  </div>
  {prediction&&<div className="banner" style={{marginTop:"14px"}}><BrainCircuit/><div><b>{prediction.is_anomaly?"Anomaly detected":"Behavior looks normal"}</b><small>Risk score {prediction.risk_score}/100 · {prediction.risk_level} · this will be used as the employee's starting score when you save</small></div></div>}
 </Panel>
 </div>
}
...
function AlertsPage({user}){
 const [resolved,setResolved]=useState(()=>JSON.parse(localStorage.getItem("sq_resolved_alerts")||"[]"));
 const [showResolved,setShowResolved]=useState(false);
 const setAndSave=list=>{setResolved(list);localStorage.setItem("sq_resolved_alerts",JSON.stringify(list))};
 const resolveAlert=id=>{setAndSave([...new Set([...resolved,id])]);toast("Alert marked as resolved.")};
 const reopenAlert=id=>{setAndSave(resolved.filter(x=>x!==id));toast("Alert reopened.")};
 const visible=alerts.filter(a=>showResolved||!resolved.includes(a.id));
 ...
 <Panel>{visible.map(a=>{const isResolved=resolved.includes(a.id);return <div className="alert-card" key={a.id}><div className="alert-symbol"><TriangleAlert/></div><div><b>{a.title}</b><p>{a.desc}</p><small>{a.employeeId} · {a.employee}</small></div>{isResolved?<Pill status="Resolved"/>:<Pill status={a.severity}/>}<Link className="secondary" to={"/investigation/"+a.id}>Investigate</Link>{user.role==="Admin"&&(isResolved?<button className="secondary" onClick={()=>reopenAlert(a.id)}>Reopen</button>:<button className="primary" onClick={()=>resolveAlert(a.id)}>Resolve</button>)}</div>})}
...
function RiskPage(){
 ...
 const all=[...custom,...seed].filter(e=>!removed.includes(e.id));
 const byDept={};
 all.forEach(e=>{if(!byDept[e.dept])byDept[e.dept]={total:0,count:0};byDept[e.dept].total+=e.score;byDept[e.dept].count++});
...
function Reports(){
 const reportData={
  "Weekly Threat Summary":{file:"weekly_threat_summary.csv",rows:[["Alert","Employee","ID","Severity"],...alerts.map(a=>[a.title,a.employee,a.employeeId,a.severity])]},
...
function Investigation({id}){
 ...
 <Panel><div className="incident"><div><small>RISK SCORE</small><b>{inc.riskScore}</b><span>{inc.confidence}% confidence</span></div><div><label>THREAT ASSESSMENT</label><h2>{inc.assessment}</h2><p>{inc.assessmentDetail}</p></div></div></Panel>
 <Panel><label>THREAT STORY · {inc.employee} ({inc.employeeId})</label>{inc.timeline.map(x=><div className="story" key={x[0]+x[1]}><Clock size={15}/><div><b>{x[0]} · {x[1]}</b><p>Evidence preserved for analyst review and case correlation.</p></div></div>)}</Panel>
 </div>
}
...
function App(){
 ...
 let page=p==="/"?<Dashboard user={user}/>
  ...
  :p.startsWith("/investigation")?<Investigation id={p.split("/")[2]}/>
